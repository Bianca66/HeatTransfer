...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void copy_values_from_previous_step(float* in, float* ct) {
	for (long long i = omp_get_thread_num() * DIM * DIM / MAX_THREADS; i < omp_get_thread_num() * DIM * DIM / MAX_THREADS + DIM * DIM / MAX_THREADS; ++i) {
		if (ct[i] != T_MIN)
			in[i] = ct[i];
	}
}
...
{
	// Find pixel position
	for (long long a = omp_get_thread_num() * DIM * DIM / MAX_THREADS; a < omp_get_thread_num() * DIM * DIM / MAX_THREADS + DIM * DIM / MAX_THREADS; ++a) {
		int i = a / DIM;
		int j = a % DIM;
